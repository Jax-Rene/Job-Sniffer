...
object JobAnalysis {
  ...
  def main(args: Array[String]) {
    ...
    //初始化
    JobEnum.listAllJobs().toArray().iterator.foreach(i => {
      jobMap += (i.toString -> new JobAnalysis(i.toString))
    })
    //记载所有Area分析结果 TODO 接下来这个可以直接顺序下来不用特殊加载 这里先用于测试
    var areaMap: Map[String, AreaAnalysis] = Map()
    areas.iterator.foreach(i => areaMap += (i -> AnalysisDao.loadAreaAnalysis(i)))
  ...
  def calcJob(min: Long, max: Long, sc: SparkContext, areas: Array[String],
              ...
              map: Map[String, JobAnalysis]): Unit = {
    val rdd = new JdbcRDD(
      sc,
      () => {
    ...
    JobEnum.values().foreach(job => {
      ...
      map(job.getName).setEducation(outPutJson(educationMap))
      //工作年限
      val workYearMap = jobRdd.map(line => (line._6, 1)).reduceByKey((a, b) => (a + b)).collect().toMap
      map(job.getName).setWorkYear(outPutJson(workYearMap))
      //需求公司类别
      ...
      map(job.getName).setIndustryField(outPutJson(industryMap))
      //公司规模
      ...
      map(job.getName).setFinanceStage(outPutJson(financeStageMap))
      //工作在不同城市的不同需求
